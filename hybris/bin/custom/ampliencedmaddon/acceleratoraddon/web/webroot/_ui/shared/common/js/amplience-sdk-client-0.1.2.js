...
(function(){
     ...
     */
    ampx.init = function(conf) {
        for (var i in conf) {
            if (ampx.conf.hasOwnProperty(i)){
                ampx.conf[i] = conf[i];
            }
        }
 ...
 * @param {Integer}
 */
var indexOf = ampx.indexOf = function(arr, elt /*, from*/)
{
    "use strict";
    if (arr == null) {
        throw new TypeError();
 ...
 */
ampx.getAssetURL = function (asset) {
    return ampx.conf.di_basepath + asset.type + '/' + ampx.conf.client_id + '/' + asset.name;
};
    ...
    for(var i in data) {
        if(data.hasOwnProperty(i)){
            ...
            for(var m =0; m<v.media.length; m++) {
                v.media[m].htmlCodec = ampx.videoToFormat(v.media[m]);
            }
        }
...
var orderVideoSources = function(data,order) {
    for(var i in data) {
        if(data.hasOwnProperty(i)){
            var v = data[i];
            v.media = ampx.sortVideoSource(v.media,order);
        }
 ...
 */
ampx.get = function (assets, success, error, videoSort) {
    ...
    var win = function(url){
        return function (name,data) {
            ...
            if(notLoaded.length){
                ampx.get(notLoaded,function(vData) {
    ...
    if(!isArray(assets)){
        ...
        if(!isValid(assets))
            return;
        var url = ampx.getAssetURL(assets);
        jsonp(ampx.getAssetURL(assets)+ '.js', assets.name, win(url), fail(url),assets.transform);
    }else{
        assLength = assets.length;
        for (var i = 0; i < assLength; i++) {
            if(!isValid(assets[i]))
                continue;
            var url = ampx.getAssetURL(assets[i]);
...
var movingCacheWindow = function () {
    ...
    timestamp -= timestamp % ampx.conf.cache_window;
    return timestamp;
};
 ...
 */
ampx.jsonReturn = function(name, data) {
...
ampx.get.createScript = function(src, onerror) {
 ...
 */
ampx.clearJsonCache = function(){
    ampx.jsonCache = {};
}

var jsonp =  ampx.jsonp = function(url, name, success, error, transform){
    ...
    if(ampx.jsonCache[name]) {
        success(name, ampx.jsonCache[name]);
        return;
    }
    ...
    cbTimeout[name] = setTimeout(function() {
        ampx.jsonReturn(name,{ status:'error',code: 404, message: "Not Found", name: name });
    }, 10000);

    var src = url + "?" + transform + buildQueryString({deep:true, timestamp: movingCacheWindow(), arg: "'"+name+"'", func:"ampx.jsonReturn"});
    var script = ampx.get.createScript(src, function(e) {
        ampx.jsonReturn(name,{ status:'error',code: 404, message: "Not Found", name: name });
    });
...
(function () {
    ...
    ampx.content = function (assets, win, fail) {
    ...
    var buildContentUrl = function (name) {
        return ampx.conf.content_basepath + 'c/' + ampx.conf.client_id + '/' + name+'.js';
    };

    var buildPayloadUrl = function (assets) {
        return ampx.conf.content_basepath + 'p/' + ampx.conf.client_id + '/[' + generateContentArray(assets) + '].js';
    };
    ...
    var payloader = function(assets,finished) {
        ...
        for(var i=0;i<it;i++) {
            ...
            ampx.jsonp(buildPayloadUrl(assets),array.join(','),onWin,onFail);
        }
    };
 ...
 */
ampx.genHTML = function (asset, attach, lazy, videoSourceSort) {
    if(!asset) return;
    var doms = {};
...
ampx.videoToFormat = function(video){
    if(!video) return;
...
ampx.genVideoHTML = function(asset,  videoSourceSort){
    if(!asset.src) {
        asset.src=asset.url;
    }
    ...
    if(!hasSize(asset)) {
        video.setAttribute('poster', ampx.di.width( asset.src, ampx.conf.default_size));
    } else {
    ...
    if(videoSourceSort){
        sorted = ampx.sortVideoSource(media, videoSourceSort);
    }else{
        sorted = media;
    }
    if(sorted) {
        for (var m=0;m<sorted.length;m++) {
            ...
            src.setAttribute('type', ampx.videoToFormat(media));
...
ampx.sortVideoSource = function(media, by){
...
var genAssetHTML = function (asset,lazy, videoSourceSort) {
    var dom;
     if (asset.isImage || asset.type == "img") {
        ...
        dom = ampx.genVideoHTML(asset, videoSourceSort);
...
var webCacheSize = function (data) {
    if (data.width > data.height) {
        return 'w='+ampx.conf.default_size;
    } else {
        return 'h='+ampx.conf.default_size;
    }
};
...
(function () {
     ...
     */
    ampx.di.set = function (src, options, filter, caller) {
        if(options){
             !isArray(options) ?
    ...
    *  padd
    ...
    (function buildDIOptions() {
        for (var i = 0; i < diOptions.length; i++) {
            (function (options) {
                ampx.di[options.fullName] = function (src, option) {
                    if(!src) return;
                    ...
                    if(typeof(opt)!="undefined"){
                        ...
                        return ampx.di.set(src, newObj, null, sn);
                    }
                    var val = ampx.di.set(src, null, null, sn);
                    switch (options.type) {
                        case 'number':
                            return Number(val);
                        case 'boolean':
 ...
 */
ampx.stats.event = function(dom,type,event,value){
    var cbs = [];
...
(function ( $ ) {
    $.widget( "ampx.ampStack", {
        ...
        _track: function(event,value) {
            ...
            if(window.ampx && ampx.stats && ampx.stats.event){
                ampx.stats.event(this.element,'stack',event,value);
            }
        },
        _destroy: function() {
...
(function ( $ ) {
    ...
    $.widget( "ampx.ampxCarousel",$.ampx.ampStack, {
        options:{
            animDuration : 250,
            easing : '',
            dir : 'horz',
        ...
        _getCreateOptions:function(){
            var attributes = this.element.data().ampxCarousel;
            if (attributes) {
                return $.extend(true, {}, this.options, attributes);
            }
            return this.options;
        ...
        _track: function(event,value) {
            this._trigger( event, null, value );
            if(window.ampx && ampx.stats && ampx.stats.event){
                ampx.stats.event(this.element,'carousel',event,value);
            }
        },
        _destroy: function() {
            this.element.removeClass('amp');
    ...
    (function(){
        ...
        $.ampx.ampxCarousel.prototype._layoutManagers['standard'] = {'requiredFeatures':null,'create':standard};
    }());
    ...
    (function(){
        ...
        $.ampx.ampxCarousel.prototype._layoutManagers['carousel3D'] = {'requiredFeatures':['can3D','transform'],'create':carousel3D};
    }());
}( jQuery ));
(function ( $ ) {

    $.widget( "ampx.ampImage", {
        options: {
            errImg: null,
            preload:'visible',
            insertAfter:false
        ...
        _failLoad: function() {
            if(this.options.errImg) {
                ...
                if (window.ampx && ampx.conf && ampx.conf.err_img) {
                    this.element.attr('src',ampx.conf.err_img);
                }
            }
        },
        _track: function(event,value) {
            this._trigger( event, null, value );
            if(window.ampx && ampx.stats && ampx.stats.event){
                ampx.stats.event(this.element,'image',event,value);
            }
        },
        _destroy: function() {
            this.element.removeClass('amp');
...
(function ( $ ) {
    $.widget( "ampx.ampBuild", {
        // Default options.
        options: {
        },
        _create: function() {
            for(var prop in $.ampx){
                if($.ampx.hasOwnProperty(prop)){
        ...
        _track: function(event,value) {
            this._trigger( event, null, value );
            if(window.ampx && ampx.stats && ampx.stats.event){
                ampx.stats.event(this.element,'build',event,value);
            }
        }
    });
...
(function ( $ ) {
    ...
    $.widget( "ampx.ampNav", {
        ...
        _track: function(event,value) {
            if(window.ampx && ampx.stats && ampx.stats.event){
                ampx.stats.event(this.element,'navigation',event,value);
            }
        }
    });
...
(function ($) {
    ...
    $.widget("ampx.ampZoom", {
        ...
        _getWidth: function(url){
            if(window.ampx && ampx.di)
                return ampx.di.width(url);
        ...
        _track: function(event,value) {
            this._trigger( event, null, value );
            if(window.ampx && ampx.stats && ampx.stats.event){
                ampx.stats.event(this.element,'zoom',event,value);
            }
        },
...
(function ($) {
    ...
    $.widget("ampx.ampVideo", {
        ...
        _track: function (event, value) {
            this._trigger(event, null, value);
            if(window.ampx && ampx.stats && ampx.stats.event){
                ampx.stats.event(this.element, 'video', event, value);
            }
        },
        _destroy: function() {
...
(function ( $ ) {
    ...
    $.widget( "ampx.ampSpin",$.ampx.ampStack, {
        options:{
            delay: 50,
            autoplay:false,
            loop:true,
        ...
        _track: function(event,value) {
            this._trigger( event, null, value );
            if(window.ampx && ampx.stats && ampx.stats.event){
                ampx.stats.event(this.element,'spin',event,value);
            }
        },
        _destroy: function() {
            this.element.removeClass('amp');
