...
{
	private static final Logger LOG = LoggerFactory.getLogger(DefaultAmplienceSeoImageNameStrategy.class);
	private static final String CACHE_KEY_PREFIX = DefaultProductModelUrlResolver.class.getName();
	private static final Pattern patternInvalidCharacters = Pattern.compile("[^%A-Za-z0-9\\-]+");
	...
	{
		return CACHE_KEY_PREFIX + "." + source.getPk().toString();
	}
	...
	protected String urlSafe(final String text)
	{
		...
		cleanedText = patternInvalidCharacters.matcher(cleanedText).replaceAll("-");
		return cleanedText;
	}
}
